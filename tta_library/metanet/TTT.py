...
class TTTBase(nn.Module):
    def __init__(self, config: TTTConfig, layer_idx: Optional[int] = None, input_dim: Optional[int] = None, output_dim: Optional[int] = None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
    ...
    def _init_qkvo_proj(self):
        self.q_proj = nn.Linear(self.input_dim, self.num_heads * self.head_dim, bias=False)
        # we share Q/K projection when using Mamba backbone
        if not self.share_qk:
            self.k_proj = nn.Linear(self.input_dim, self.num_heads * self.head_dim, bias=False)
        self.v_proj = nn.Linear(self.input_dim, self.num_heads * self.head_dim, bias=False)
...
class TTTMGG(nn.Module):
    def __init__(self, config: TTTConfig, layer_idx: int, input_dim: int, output_dim: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.seq_modeling_block = ttt_layer(config=config, layer_idx=layer_idx, input_dim=input_dim, output_dim=output_dim)
    ...
    def forward(
        ...
        return torch.tanh(hidden_states), last_mini_batch_params_dict # Add activation function tanh
...
if __name__ == "__main__":
    ...
    configuration = TTTConfig(hidden_size=8, intermediate_size=0, num_hidden_layers=1, num_attention_heads=1, mini_batch_size=1,)
    ...
    for _ in range(50):
        ...
        outputs, last_mini_batch_params_dict = ttt_mgg(inputs, last_mini_batch_params_dict=None)
        optimizer.zero_grad()
