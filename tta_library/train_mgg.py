...
class TrainMGG(nn.Module):
    ...
    """
    def __init__(self, model:PromptViT, mgg:nn.Module, mgg_optimizer, adapt_lr=1, beta1=0.9, beta2=0.99, fitness_lambda=0.4, norm_dim=2):
        super().__init__()

        self.model = model
        self.mgg = mgg

        self.mgg_optimizer = mgg_optimizer
        ...
        self.mgg_state, self.optimizer_state = \
            copy_model_and_optimizer(self.mgg, self.mgg_optimizer)
    ...
    def init_state(self):
        self.grad_m = {}
        self.grad_v = {}
        self.last_mini_batch_params_dict = {}
    ...
    def _hook_generator(self, idx, name):
        def continue_backward_hook(grad):
            self.result_params_list[idx][name].backward(grad, retain_graph=True)
        return continue_backward_hook

    def _remove_hook(self):
        for handel in self.hook_handle:
            for name in self.norm_param_names:
                handel[name].remove() 
        self.hook_handle = []
    ...
    def forward(self, x, y):
        ...
        outputs, loss, _, batch_mean = forward_and_get_loss(x, y, self.model, self.imagenet_mask, self.fitness_lambda, self.train_info)
        # batch_mean = batch_mean[-768:].unsqueeze(0)
        
        loss.backward()
        
        # update MGG parameters
        self.mgg_optimizer.step()
        self.mgg_optimizer.zero_grad()
        ...
        for (name, p) in self.model.named_parameters():
            if name in self.norm_param_names:
                ...
                mt_tilde = mt_tilde.view(self.norm_dim, 1, -1) #(3,1,256)
                gt_tilde = gt_tilde.view(self.norm_dim, 1, -1) #(3,1,256)

                mgg_inputs = torch.cat([mt_tilde, gt_tilde], dim=2) #(3,1,256*2)
                mgg_inputs_list.append(mgg_inputs)
                
          
        mgg_inputs = torch.cat(mgg_inputs_list, dim=0)
        # print(f'mgg_inputs.shape: {mgg_inputs.shape}')        
        ...
        # Create hooks to trace parameter updates for gradient backpropagation to MGG
        self.result_params_list.append(result_params) 
        idx = len(self.result_params_list) - 1
        handel = {}
        for name in result_params:
            rsetattr(self.model, name, nn.Parameter(result_params[name].clone().detach()))
            hook_funciton = self._hook_generator(idx, name)
            handel[name] = rgetattr(self.model, name).register_hook(hook_funciton)
        self.hook_handle.append(handel)
    ...
    def reset(self):
        #mgg reset
        load_model_and_optimizer(self.mgg, self.mgg_optimizer,
                                 self.mgg_state, self.optimizer_state)
        
        self.init_state()
        
    def configure_model(self):
        """Configure model for use."""
        # train mode, because mgtta optimizes the model to minimize entropy
        self.model.train()
        # disable grad, to (re-)enable only what updates
    ...
    def get_mgg_clone(self):
        return deepcopy(self.mgg)
...
def forward_and_get_loss(images, target, model:PromptViT, imagenet_mask, fitness_lambda, train_info):
    ...
    return output, loss, cls_features_mean, batch_mean
...
def create_mgg(hidden_size=768, num_attention_heads=1):
    input_dim = 2 
    output_dim = 1
