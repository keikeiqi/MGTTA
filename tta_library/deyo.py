...
class DeYO(nn.Module):
    ...
    """
    def __init__(self, model, args, optimizer, steps=1, episodic=False, deyo_margin=0.5, margin_e0=0.4):
        super().__init__()
        self.model = model
        ...
        self.deyo_margin = deyo_margin * math.log(1000)
        self.margin_e0 = margin_e0 * math.log(1000)
    ...
    def forward(self, x, targets=None, flag=True, group=None):
        ...
        if targets is None:
            for _ in range(self.steps):
                if flag:
                    outputs, backward, final_backward = forward_and_adapt_deyo(x, self.model, self.imagenet_mask, self.args,
                                                                              self.optimizer, self.deyo_margin,
                                                                              self.margin_e0, targets, flag, group)
                else:
                    outputs = forward_and_adapt_deyo(x, self.model, self.imagenet_mask, self.args,
                                                    self.optimizer, self.deyo_margin,
                                                    self.margin_e0, targets, flag, group)
        else:
            for _ in range(self.steps):
                if flag:
                    outputs, backward, final_backward, corr_pl_1, corr_pl_2 = forward_and_adapt_deyo(x, self.model, self.imagenet_mask, 
                                                                                                    self.args, 
                                                                                                    self.optimizer, 
                                                                                                    self.deyo_margin,
                                                                                                    self.margin_e0,
                    ...
                    outputs = forward_and_adapt_deyo(x, self.model, self.imagenet_mask,
...
def forward_and_adapt_deyo(x, model, imagenet_mask, args, optimizer, deyo_margin, margin, targets=None, flag=True, group=None):
    ...
    if imagenet_mask is not None:
        ...
        deyo_margin = deyo_margin / math.log(1000) * math.log(200)
        margin = margin / math.log(1000) * math.log(200)
