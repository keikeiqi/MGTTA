...
class EATA(nn.Module):
    ...
    def __init__(self, model, optimizer, fishers=None, fisher_alpha=2000.0, steps=1, episodic=False, e_margin=math.log(1000)/2-1, d_margin=0.05):
        ...
        self.e_margin = e_margin * math.log(1000) # hyper-parameter E_0 (Eqn. 3)
    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(x, self.model, self.imagenet_mask, self.optimizer, self.fishers, self.e_margin, self.current_model_probs, fisher_alpha=self.fisher_alpha, num_samples_update=self.num_samples_update_2, d_margin=self.d_margin)
...
def forward_and_adapt_eata(x, model, imagenet_mask, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, scale_factor=2, num_samples_update=0):
    ...
    if imagenet_mask is not None:
        ...
        d_margin = d_margin / math.log(1000) * math.log(200)
    # adapt
